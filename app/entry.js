...
socket.on('map data', (compressed) => {
  ...
  const flyingMissileArray = compressed[3];
  ...
  for(let compressedPlayerData of playersArray) {
    ...
    flyingMissileArray.forEach((compressedFlyingMissileData, index) => {
...
$(window).on('keydown', (event) => {
  ...
  switch(event.key) {
    ...
    case 'ArrowRight':
      ...
      break;
    case ' ': //スペースキー
      if(gameObj.myPlayerObj.missilesMany <= 0) break; //ミサイルの数がゼロ

      gameObj.myPlayerObj.missilesMany -= 1;
      const missileId = Math.floor(Math.random() * 100000) + ',' + gameObj.myPlayerObj.socketId
      + ',' + gameObj.myPlayerObj.x + ',' + gameObj.myPlayerObj.y;
...
function moveInClient(myPlayerObj, flyingMissilesMap) {
  ...
  if(myPlayerObj.aliveTime.clock === 30) {
    ...
    myPlayerObj.aliveTime.second += 1;
  }
