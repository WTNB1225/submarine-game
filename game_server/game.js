...
const gameTicker = setInterval(() => {
  ...
  checkGetItem(playersAndNPCMap, gameObj.itemMap, gameObj.airMap, gameObj.flyingMissilesMap);
  addNPC();
}, 33);
...
function newConnection(socketId, displayName, thumbUrl) {
  ...
  const startObj = {
    ...
    missiileSpeed: gameObj.missileSpeed
  };
  return startObj;
}
...
function getMapData() {
  ...
  return [playersArray, itemArray, airArray, flyingMissilesArray]
}
...
function missileEmit(socketId, direction) {
  ...
  if(emitPlayerObj.isAlive === false) return; //死んでいる
...
function moveMissile(flyingMissileMap){ //ミサイルの移動
  for(let [missileId, flyingMissile] of flyingMissileMap) {
    const missile = flyingMissile;

    if(missile.aliveFlame === 0) {
      flyingMissileMap.delete(missileId)
    }
...
function checkGetItem(playersMap, itemMap, airMap, flyingMissilesMap) {
  for(let [hashKey, playerObj] of playersMap) {
    if(playerObj.isAlive === false) continue;

    //アイテムミサイル 赤丸
    for(let [itemKey, itemObj] of itemMap) {
      const distanceObj = calculationBetweenTwoPoints( 
        ...
        if (
          ...
          gameObj.itemMap.delete(itemKey);
